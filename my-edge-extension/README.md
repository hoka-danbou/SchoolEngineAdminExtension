...
- `src/content/index.js` は **classic script** として読み込まれます。
- そのため static `import` は使わず、`import(chrome.runtime.getURL(...))` の dynamic import で `src/content/main.js` を起動します。
- これにより `Cannot use import statement outside a module` エラーを回避できます。
